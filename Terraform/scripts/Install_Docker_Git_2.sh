		...
		ubuntu|debian|raspbian)
			...
			(
				...
				if [ -n "$cli_pkg_version" ]; then
					$sh_c "apt-get install -y -qq --no-install-recommends docker-ce-cli=$cli_pkg_version "
				fi
				$sh_c "apt-get install -y -qq --no-install-recommends docker-ce$pkg_version "
			)
			echo_docker_as_nonroot
			exit 0
			;;
